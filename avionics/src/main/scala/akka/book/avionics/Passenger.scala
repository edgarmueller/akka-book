...
object Passenger {
  // These are notifications that tell the Passenger to
  // fasten or unfasten their seat belts
...
// The DrinkRequestProbability trait defines some thresholds
// that we can modify in tests to speed things up.
trait DrinkRequestProbability {
  ...
  // Some portion of this (0 to 100 percent) is added on to
  // requestMin
...
class Passenger(callButton: ActorRef) extends Actor
  ...
  // since our Flight Attendants have been coded to serve
  // them up
...
//    val drinks = List("Wine", "Beer", "Water").toIndexedSeq
  val drinks = 
  context.system.settings.config.getStringList(
    "akka.book.avionics.drinks").asScala.toIndexedSeq
    drinks map (log.info _)

  // A shortcut for the scheduler to make things look nicer
  // later
  ...
  // to get a drink using some randomness to decide
